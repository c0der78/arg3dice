...
namespace coda {
  namespace yaht {
    ...
    scoresheet::scoresheet(const scoresheet &other)
        : upperPlayed_(other.upperPlayed_), lowerPlayed_(other.lowerPlayed_),
    ...
    void scoresheet::upper_score(die::value_type index,
                                 scoresheet::value_type value, bool played) {
    ...
    scoresheet::upper_score(die::value_type index) const {
      auto i = std::max(0U, index - 1);
