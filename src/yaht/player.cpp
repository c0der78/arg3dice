...
namespace coda {
  namespace yaht {
    ...
    player::player(die::engine *const engine)
        : dice_(die_count(), die_sides(), engine), score_(), rollCount_(0) {}

    player::player(const player &other)
        : dice_(other.dice_), score_(other.score_),
          rollCount_(other.rollCount_), kept_(other.kept_) {}

    player::player(player &&other)
        : dice_(std::move(other.dice_)), score_(std::move(other.score_)),
          rollCount_(other.rollCount_), kept_(std::move(other.kept_)) {}
    ...
    unsigned int player::die_sides() const { return die::DEFAULT_SIDES; }

    unsigned int player::die_count() const { return Constants::NUM_DICE; }

    unsigned short player::roll_count() const { return rollCount_; }
    ...
    void player::roll() {
      dice_.roll([&](size_t index, const die &d) { return !is_kept(index); });
    ...
    const dice &player::d1ce() const { return dice_; }
    ...
    const die &player::d1e(size_t index) const { return dice_[index]; }

    scoresheet &player::score() { return score_; }

    scoresheet::value_type
    player::calculate_upper_score(die::value_type value) const {
    ...
    scoresheet::value_type
    player::calculate_number_of_a_kind(int length) const {
      auto values = dice_;
    ...
    scoresheet::value_type player::calculate_full_house() const {
      ...
      if (((values[0] == values[1]) && (values[1] == values[2]) &&
           (values[3] == values[4]) && (values[2] != values[3])) ||
    ...
    player::calculate_best_lower_score() const {
      scoresheet::value_type value = 0;
    ...
    scoresheet::value_type
    player::calculate_lower_score(scoresheet::type type) const {
      switch (type) {
